...
void main() {
  testWidgets('Arsenal Hover: dragging existing module over arsenal should NOT set overGrid to false', (tester) async {
    bool removed = false;
    final modules = [
    ...
    expect(find.byType(ZeusModuleWidget), findsNWidgets(1)); // Only the static one? 
    ...
    expect(find.byType(Container).evaluate().any((e) {
      final container = e.widget as Container;
      return container.decoration is BoxDecoration && (container.decoration as BoxDecoration).border != null;
    }), isTrue, reason: 'Ghost should still be visible even when hovering arsenal');

    await gesture.up();
    await tester.pumpAndSettle();

    expect(removed, isFalse, reason: 'Module should not be removed when hovering arsenal during drag');
  });
}
